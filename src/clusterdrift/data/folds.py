...
def compute_split_hash(
    ...
    """Compute deterministic cryptographic SHA256 digest over partition structure and indices."""
...
def generate_temporal_block_splits(
    ...
    """Generate chronological expanding-window splits with zero future leakage."""
    sorted_order = np.argsort(time_values, kind="stable")
    tscv = TimeSeriesSplit(n_splits=n_outer)
    splits: List[FoldSplit] = []

    for r, (train_sub, test_sub) in enumerate(tscv.split(sorted_order)):
        source_idx = sorted_order[train_sub]
        target_idx = sorted_order[test_sub]

        # Enforce temporal monotonicity
        max_source_time = time_values[source_idx].max()
        min_target_time = time_values[target_idx].min()
        if max_source_time > min_target_time:
            raise ValueError(f"Temporal leakage in fold {r}: max_source ({max_source_time}) > min_target ({min_target_time})")

        # Inner chronological splits within outer source
        inner_tscv = TimeSeriesSplit(n_splits=n_inner)
        inner_folds: List[InnerFold] = []
        for q, (inner_train_sub, inner_val_sub) in enumerate(inner_tscv.split(train_sub)):
            in_train = sorted_order[train_sub[inner_train_sub]]
            in_val = sorted_order[train_sub[inner_val_sub]]
            inner_folds.append(InnerFold(inner_fold=q, train_indices=in_train, val_indices=in_val))
        ...
        splits.append(
            FoldSplit(
                ...
                metadata={
                    ...
                    "max_source_time": float(max_source_time),
                    "min_target_time": float(min_target_time),
                    "notes": "Chronological expanding-window split; contains genuine temporal structure.",
                },
            )
        )
    return splits
...
def generate_tableshift_natural_splits(
    ...
    for ood_val in candidate_ood_values:
        ...
        target_indices = np.where(domain_series == ood_val)[0]
        source_all_indices = np.where(domain_series != ood_val)[0]
        ...
        for r, (train_sub, excluded_sub) in enumerate(kf.split(source_all_indices)):
            source_train_idx = source_all_indices[train_sub]
            source_excluded_idx = source_all_indices[excluded_sub]
            ...
            splits.append(
                FoldSplit(
                    ...
                    split_strategy="natural_domain",
                    metadata={
                        "admission_source_id": int(ood_val),
                        "split_seed": split_seed,
                        "source_excluded_rows": len(source_excluded_idx),
                    },
                )
...
def verify_split_artifact(
    ...
    """Verify cryptographic integrity and provenance of existing split artifacts."""
    ...
    try:
        ...
        if expected_bundle_sha256 is not None:
            if meta.get("canonical_bundle_sha256") != expected_bundle_sha256:
                return False
        ...
        recomputed_hash = compute_split_hash(
            ...
            canonical_bundle_sha256=meta["canonical_bundle_sha256"],
        )
