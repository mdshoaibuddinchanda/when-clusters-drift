...
│   ├── controlled/           # 30 controlled real datasets (features.parquet, labels.parquet, metadata.json)
│   └── natural/              # Natural-shift datasets separated by domain/state
│       ├── whyshift/
│       └── tableshift/
│
...
## 5. Physical Label Separation & Leakage Prevention

Every canonicalized dataset is split into separate files:
- `features.parquet`: Pure input features $\tilde{X}$. The target column $y$ is strictly omitted (`assert target_column not in X.columns`).
- `labels.parquet`: Evaluation ground truth $y$. Kept isolated from deployment code.
- `metadata.json`: Feature names, row counts, domain indicators, and license provenance.
